...
half_lightsum_lim=15 # Gaismas summas limits, pie kuras atslēgsies gaisma
half_off_tresh=400 # Izslēgšanās limits
half_on_tresh=280 # atpakaļieslēgšanās limits

all_lightsum_lim=12 #
all_off_tresh=120 # Izslēgšanās limits
all_on_tresh=40
...
nat_compare=0
lightsum=0#  kopējā gaismas summa
...
start_hour=5 # Stunda cikos sākam gaismot -
start_minute=56 # Minūte cikos sākam gaismot
end_hour=5 #Stunda, ckios beidzam gaismot
end_minute=59  #Minūte, kurā beidzam stundas robežās gaismot
all_start_delay=1 # 100% ieslēgšanās aizture
all_end_delay=1 # 100% izslēgšanās pirms visu lampu izslēgšanās
...
def counter_reset():
    ...
    finame="/home/juska/log/"+date[5:10]+".txt"
...
def write_log():
    ...
    global all_lightsum_lim
    global all_GPIO
    inwrite=str(lightsum) + ','+ str(nat_compare)+','+ str(half_light_status_on)+','+ str(half_off_tresh)+','+ str(half_off_counter)+','+ str(half_on_tresh)+','+ str(half_on_counter)+','+ str(half_GPIO)+','+ str(all_light_status_on)+','+ str(all_off_tresh)+','+ str(all_on_counter)+','+ str(all_GPIO)
